...
// reliable answer to "which order does this part put a card on screen for".
describe("orderCardOrderId", () => {
  it("reads the order from checkout_submit", () => {
    expect(orderCardOrderId("checkout_submit", { order_id: "84aa1a40" })).toBe("84aa1a40");
  });
  ...
  it("gives both tools the same id for the same order", () => {
    expect(orderCardOrderId("order_status", { order: { id: "84aa1a40" } })).toBe(
      orderCardOrderId("checkout_submit", { order_id: "84aa1a40" }),
    );
  });
  ...
  it("returns nothing for a missing or non-string id", () => {
    expect(orderCardOrderId("checkout_submit", undefined)).toBeUndefined();
    expect(orderCardOrderId("checkout_submit", {})).toBeUndefined();
